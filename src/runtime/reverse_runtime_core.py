"""Generic reverse-analysis runtime core and tool execution extensions."""
...
class MutationTrackingToolExecutionExtension:
    """Adds mutation_effective annotations and counters for reverse-analysis sessions."""
...
class BaseReverseRuntimeCore:
    """Shared LLM-driven reverse-analysis runtime with a single tool-call policy loop."""
    ...
    def __init__(
        ...
        model_name = str(model or os.getenv("OPENAI_MODEL", self.DEFAULT_MODEL)).strip()
        if not model_name:
...
PROFILE_SYSTEM_PROMPTS: Dict[str, str] = {
    "struct_recovery": "agent/reverse_expert_system.md",
    "attack_surface": "agent/attack_surface_system.md",
    "general_reverse": "agent/general_reverse_system.md",
}


class ReverseToolExecutionExtension(MutationTrackingToolExecutionExtension):
    """Profile-aware mutation tracker used by the generic reverse runtime."""

    def __init__(
        self,
        mutating_tool_names: Optional[set[str]] = None,
        type_application_tool_names: Optional[set[str]] = None,
    ) -> None:
        self.mutating_tool_names = set(mutating_tool_names or set())
        self.type_application_tool_names = set(type_application_tool_names or {"set_identifier_type"})
        self.reset()
...
class ReverseRuntimeCore(BaseReverseRuntimeCore):
    """Generic LLM-driven reverse-analysis runtime reused by all main profiles."""
    ...
    def __init__(
        ...
        runtime_name: str = "ReverseRuntimeCore",
        tool_execution_extension_factory: Optional[Callable[[], Any]] = None,
        finalize_tool_name: str = "",
        system_prompt_path: Optional[str] = None,
    ):
        selected_profile = normalize_tool_profile(agent_profile or tool_profile or "struct_recovery")
        finalize_config = get_finalize_config_for_profile(selected_profile)
        self.agent_profile = selected_profile
        self.finalize_config = dict(finalize_config)
        self.finalize_tool_name = str(finalize_tool_name or self.finalize_config.get("tool_name", "")).strip()
        if not self.finalize_tool_name:
            self.finalize_tool_name = "submit_output"
        self.finalize_config["tool_name"] = self.finalize_tool_name
        self.system_prompt_path = str(
            system_prompt_path
            or PROFILE_SYSTEM_PROMPTS.get(selected_profile, "agent/reverse_expert_system.md")
        ).strip()

        extension_factory = tool_execution_extension_factory
        if extension_factory is None:
            mutating = get_mutating_tools_for_profile(selected_profile)

            def _factory() -> ReverseToolExecutionExtension:
                return ReverseToolExecutionExtension(mutating_tool_names=mutating)

            extension_factory = _factory
        ...
        super().__init__(
            ...
            tool_profile=normalize_tool_profile(tool_profile or selected_profile),
            runtime_name=str(runtime_name or "").strip() or "ReverseRuntimeCore",
            tool_execution_extension_factory=extension_factory,
        )
        self.agent_profile = selected_profile
        self.finalize_tool_name = self.finalize_config["tool_name"]
        self.system_prompt_path = self.system_prompt_path
        self.tool_profile = normalize_tool_profile(tool_profile or selected_profile)
        self.prompt_manager.validate_required([self.system_prompt_path])

    def _finalize_tool_names(self) -> set[str]:
        return {
            str(self.finalize_tool_name or "").strip(),
            "submit_subagent_output",
            "submit_idapython_result",
        }

    def _accept_main_finalize(self, payload: Dict[str, str]) -> str:
        pending = self.subagent_mgr.pending_for_parent(parent_agent_id="main")
        if pending:
            lines = [
                f"ERROR: {self.finalize_tool_name} blocked by running subagents",
                f"- pending_count: {len(pending)}",
                "- pending_subagents:",
            ]
            for state in pending:
            ...
            lines.append(f"- action: wait for spawn_subagent results before {self.finalize_tool_name}")
            return "\n".join(lines)
        self._finalized = True
        self._finalize_payload = {key: str(value or "").strip() for key, value in payload.items()}
        self._final_text = self._build_final_text(self._finalize_payload)
        return f"OK: {self.finalize_tool_name} accepted"
    ...
    def _make_runtime_tools(
        ...
        *,
        ...
        base_tools = super()._make_runtime_tools(
            current_agent_id=current_agent_id,
            parent_agent_id=parent_agent_id,
            user_request=user_request,
            max_iterations=max_iterations,
        ...
        base_tools = [tool_obj for tool_obj in base_tools if str(getattr(tool_obj, "name", "") or "") != "submit_output"]

        if self.finalize_tool_name == "submit_attack_surface_output":
            @tool("submit_attack_surface_output", parse_docstring=True, error_on_invalid_docstring=True)
            def submit_attack_surface_output(
                summary: str,
                entry_points: str = "",
                interface_classification: str = "",
                risk_assessment: str = "",
            ) -> str:
                """Submit final attack-surface analysis and end the main policy loop.

                Args:
                    summary: 攻击面摘要与范围限制。
                    entry_points: 入口点列表，包含函数、地址、触发方式、外部参数。
                    interface_classification: 接口分类汇总。
                    risk_assessment: 风险等级与原因。

                Returns:
                    纯文本确认信息。
                """
                return self._accept_main_finalize(
                    {
                        "summary": summary,
            ...
            base_tools.append(submit_attack_surface_output)
            return base_tools

        if self.finalize_tool_name == "submit_reverse_analysis_output":
            @tool("submit_reverse_analysis_output", parse_docstring=True, error_on_invalid_docstring=True)
            def submit_reverse_analysis_output(
                summary: str,
                function_list: str = "",
                attack_surfaces: str = "",
                external_interactions: str = "",
                external_parameters: str = "",
                key_functions: str = "",
            ) -> str:
                """Submit final general reverse-analysis output and end the main policy loop.

                Args:
                    summary: 二进制概览与核心结论。
                    function_list: 函数清单与优先级结果。
                    attack_surfaces: 攻击面与入口点列表。
                    external_interactions: 网络、文件、注册表、进程等外部交互。
                    external_parameters: 外部参数及控制行为。
                    key_functions: 关键函数摘要。
            ...
            base_tools.append(submit_reverse_analysis_output)
            return base_tools

        @tool("submit_output", parse_docstring=True, error_on_invalid_docstring=True)
        def submit_output(summary: str, key_findings: str = "", artifacts: str = "", next_steps: str = "") -> str:
            """Submit final struct-recovery output and end the main policy loop.
    ...
    def _build_main_system_prompt(self) -> str:
        return self.prompt_manager.render(self.system_prompt_path, {})
    ...
    async def _execute_tool_calls(
        ...
        *,
        ...
        max_parallel = max(1, int(self.tool_call_max_parallel))
        sem = asyncio.Semaphore(max_parallel)
        finalize_tool_names = self._finalize_tool_names()
        ...
        outputs: List[Dict[str, Any]] = []
        if normal_calls:
            jobs = [asyncio.create_task(_run_one(idx, call)) for idx, call in normal_calls]
            outputs.extend(await asyncio.gather(*jobs))
        for idx, call in finalize_calls:
            outputs.append(await _run_one(idx, call))
        outputs = sorted(outputs, key=lambda row: int(row.get("index", 0)))
    ...
    def _build_final_text(self, payload: Optional[Dict[str, str]] = None) -> str:
        data = payload or self._finalize_payload or {}
        lines: List[str] = [str(self.finalize_config.get("title", "# Reverse Agent Final Summary")), ""]
        for heading, key in self.finalize_config.get("sections", []):
            lines.extend(
                [
                    f"## {heading}",
                    str(data.get(str(key), "")).strip() or "(empty)",
                    "",
                ]
            )
        lines.extend(
    ...
    def _build_incomplete_text(self, *, iteration: int, max_iterations: int) -> str:
        profile_name = str(self.agent_profile or "reverse").replace("_", " ").title()
        lines: List[str] = [
            f"# {profile_name} Incomplete",
            "",
            f"- iterations_used: {int(iteration)}",
            f"- max_iterations: {int(max_iterations)}",
            f"- reason: {str(self.finalize_config.get('incomplete_reason', 'max_iterations_reached'))}",
            "",
            "## Task Board",
            self.task_board.get_task_board(view="both"),
            "",

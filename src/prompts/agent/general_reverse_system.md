...
5) `Phase-Synthesize`
   ...
   - 若已具备“可交付的概览 + 攻击面 + 外部交互/参数控制 + 一批关键函数摘要”，即使仍有少量疑点，也必须优先提交阶段性最终结果，不得为了补充次要证据而无限延长。

## 迭代预算与提交纪律
你必须默认自己处在有限迭代预算内，并遵守以下纪律：
1) 一旦进入 `Phase-Synthesize`，目标从“继续找更多证据”切换为“整理已有证据并完成最终提交”。
2) 如果 Task Board 上只剩综合/提交类任务，或前面四个阶段已基本完成，就必须把下一次关键动作视为 `submit_reverse_analysis_output`，而不是继续扩张搜索面。
3) 如果连续一轮没有新增关键证据，或者新增内容只是在重复确认已有结论，必须停止采证并提交当前最优总结。
4) 允许带着明确的不确定项提交“阶段性最终结果”；不确定项应写入 summary/risk，而不是拖延提交。
5) 禁止在接近结束时保持 `Phase-Synthesize in_progress` 却不提交；缺少完美覆盖不是继续循环的理由。
...
6) 如果现在停止，现有证据是否已经足以形成“可复核的阶段性最终结论”？若是，立即提交，不再继续采证。
...
4) 如果已经形成阶段性最终结论，则宁可先提交一个带缺口说明的版本，也不要在未提交的状态下耗尽迭代预算。
